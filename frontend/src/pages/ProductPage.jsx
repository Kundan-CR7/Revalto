...
export default function ProductPage() {
  ...
  const isOwner = user && product.authorId === user.id;
          ...
          <div className="p-8 flex flex-col">
            ...
            <div className="flex gap-6 mb-8">
              ...
              <div className="border border-gray-300 rounded-lg px-6 py-3 flex-1">
                ...
                <p className="text-lg font-bold text-gray-900 mb-1 ">
                  {warrantyRemaining === "NO_WARRANTY"
                    ? "No"
                    : warrantyRemaining === "LESS_THAN_1_MONTH"
                    ? "< 1 Month"
                    : warrantyRemaining === "ONE_TO_THREE_MONTHS"
                    ? "1-3 Months"
                    : warrantyRemaining === "THREE_TO_SIX_MONTHS"
                    ? "3-6 Months"
                    : warrantyRemaining === "SIX_TO_NINE_MONTHS"
                    ? "6-9 Months"
                    : warrantyRemaining === "NINE_TO_TWELVE_MONTHS"
                    ? "9-12 Months"
                    : warrantyRemaining === "MORE_THAN_1_YEAR"
                    ? "> 1 Year"
                    : warrantyRemaining}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden shrink-0">
                  {!isPostedAnonymously && author.imgUrl ? (
                    <img
                      src={author.imgUrl}
                      alt={displayName}
                ...
                <div className="flex flex-col justify-center -space-y-1">
                  <p className="font-bold mt-2 mb-0 text-gray-900 text-base">
                    {displayName}
                  </p>
              ...
              <button
                ...
                disabled={isOwner || !isAvailable}
                className="px-6 py-3 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors font-medium disabled:bg-gray-400 disabled:cursor-not-allowed disabled:opacity-70"
              >
                {isOwner ? "YOUR PRODUCT" : "CONTACT SELLER"}
              </button>
            </div>
          </div>
        </div>
